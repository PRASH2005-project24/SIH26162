...
if not os.path.exists(weak_labels_fixed_path):
    ...
    print("[PASS] Fixed weak label file exists: " + weak_labels_fixed_path)
    df_fixed = pd.read_csv(weak_labels_fixed_path)
    print("  Rows: " + str(len(df_fixed)))
    print("  Columns: " + str(list(df_fixed.columns)))
    ...
    if missing:
        print("ERROR: Missing persistence columns: " + str(missing))
    else:
        print("[PASS] All persistence columns present.")
    # Check if the script exists
    if os.path.exists(script_path):
        print("[PASS] Generation script exists: " + script_path)
    else:
...
if len(df_fixed) >= sample_size:
    ...
    print("Warning: Using all " + str(len(df_fixed)) + " rows for temporal test.")
...
if not os.path.exists(master_path):
    ...
    print("Loading master data from " + master_path + "...")
    df_master = pd.read_csv(master_path)
    # Fix: acq_time is integer like 828 representing 08:28, need to format as HH:MM:SS
    df_master['acq_time_str'] = df_master['acq_time'].astype(str).str.zfill(4)
    df_master['acq_time_str'] = df_master['acq_time_str'].str[:2] + ':' + df_master['acq_time_str'].str[2:] + ':00'
    ...
    print("Master data rows: " + str(len(df_master)))
    ...
    for idx, row in sample_df.iterrows():
        ...
        if len(master_row) == 0:
            errors.append("Fire ID " + str(fire_id) + " not found in master data.")
            continue
        ...
        if row['persistence_date_count'] != expected_persistence_date_count:
            errors.append("Fire ID " + str(fire_id) + ": persistence_date_count mismatch. Expected " + str(expected_persistence_date_count) + ", got " + str(row['persistence_date_count']))
        if row['time_span_days'] != expected_time_span_days:
            errors.append("Fire ID " + str(fire_id) + ": time_span_days mismatch. Expected " + str(expected_time_span_days) + ", got " + str(row['time_span_days']))
        if row['detections_24h'] != expected_detections_24h:
            errors.append("Fire ID " + str(fire_id) + ": detections_24h mismatch. Expected " + str(expected_detections_24h) + ", got " + str(row['detections_24h']))
        if row['active_days'] != expected_active_days:
            errors.append("Fire ID " + str(fire_id) + ": active_days mismatch. Expected " + str(expected_active_days) + ", got " + str(row['active_days']))
    ...
    if errors:
        print("ERROR: Found " + str(len(errors)) + " temporal correctness errors.")
        for err in errors[:5]:
            print("  " + err)
    else:
        print("[PASS] Temporal correctness verified on " + str(len(sample_df)) + " random samples.")
    ...
    for fire_id in test_events:
        ...
        for ft in future_times:
            ...
            # Format time as HHMM integer for consistency with original data
            time_int = int(ft.strftime('%H%M'))
            obs['acq_time'] = time_int
            obs['acq_time_str'] = ft.strftime('%H:%M:%S')  # Keep for datetime conversion
        ...
        if orig_persistence_date_count != new_persistence_date_count:
            adversarial_errors.append("Fire ID " + str(fire_id) + ": persistence_date_count changed after adding future observations. Original " + str(orig_persistence_date_count) + ", new " + str(new_persistence_date_count))
        if orig_time_span_days != new_time_span_days:
            adversarial_errors.append("Fire ID " + str(fire_id) + ": time_span_days changed after adding future observations. Original " + str(orig_time_span_days) + ", new " + str(new_time_span_days))
        if orig_detections_24h != new_detections_24h:
            adversarial_errors.append("Fire ID " + str(fire_id) + ": detections_24h changed after adding future observations. Original " + str(orig_detections_24h) + ", new " + str(new_detections_24h))

    if adversarial_errors:
        print("ERROR: Adversarial test failed with " + str(len(adversarial_errors)) + " errors.")
        for err in adversarial_errors[:5]:
            print("  " + err)
    else:
        print("[PASS] Adversarial test passed: adding future observations did not change persistence for original events.")
...
if os.path.exists(weak_labels_old_path) and os.path.exists(weak_labels_fixed_path):
    ...
    if len(df_old) != len(df_fixed):
        print("ERROR: Row count mismatch: old=" + str(len(df_old)) + ", fixed=" + str(len(df_fixed)))
    else:
        print("[PASS] Row count matches: " + str(len(df_old)))
        ...
        for col in persistence_cols:
            if col in df_old.columns and col in df_fixed.columns:
                ...
                print("WARNING: Column " + col + " missing in one of the files.")
        print("Changed persistence column counts:")
        for col, count in changed_cols.items():
            print("  " + col + ": " + str(count) + " rows changed (" + str(count/len(df_old)*100) + "%)")
        ...
        if 'target_class' in df_old.columns and 'target_class' in df_fixed.columns:
            ...
            print("  target_class: " + str(target_changed) + " rows changed (" + str(target_changed/len(df_old)*100) + "%)")
        else:
        ...
        if 'label_confidence' in df_old.columns and 'label_confidence' in df_fixed.columns:
            ...
            print("  label_confidence: " + str(conf_changed) + " rows changed (" + str(conf_changed/len(df_old)*100) + "%)")
        else:
        ...
        if 'label_confidence_level' in df_old.columns and 'label_confidence_level' in df_fixed.columns:
            ...
            print("  label_confidence_level: " + str(conf_level_changed) + " rows changed (" + str(conf_level_changed/len(df_old)*100) + "%)")
        else:
        ...
        if 'is_persistent' in df_old.columns and 'is_persistent' in df_fixed.columns:
            ...
            print("  is_persistent: " + str(pers_changed) + " rows changed (" + str(pers_changed/len(df_old)*100) + "%)")
        else:
...
if os.path.exists(prepared_dataset_path):
    ...
    print("[PASS] Prepared dataset exists: " + str(len(df_prepared)) + " rows")
    ...
    if set(df_prepared['fire_id']) == set(df_expected['fire_id']):
        print("[PASS] Prepared dataset contains exactly the expected rows from fixed weak labels (after filtering).")
    else:
        print("ERROR: Prepared dataset row mismatch.")
        print("  Prepared: " + str(len(df_prepared)) + " rows")
        print("  Expected: " + str(len(df_expected)) + " rows")
        diff = set(df_prepared['fire_id']).symmetric_difference(set(df_expected['fire_id']))
        print("  Symmetric difference: " + str(len(diff)) + " rows")
else:
...
for name, path in [('train', train_path), ('validation', validation_path), ('test', test_path)]:
    if os.path.exists(path):
        df = pd.read_csv(path)
        print("[PASS] " + name + ".csv exists: " + str(len(df)) + " rows")
        # Check that it is a subset of the prepared dataset
        if set(df['fire_id']).issubset(set(df_prepared['fire_id'])):
            print("  [PASS] All rows in " + name + ".csv are in the prepared dataset.")
        else:
            print("  ERROR: " + name + ".csv contains rows not in prepared dataset.")
    else:
        print("ERROR: " + name + ".csv not found.")
...
if df_train is not None and df_val is not None and df_test is not None:
    ...
    if df_train_geo['latitude'].isnull().any() or df_val_geo['latitude'].isnull().any() or df_test_geo['latitude'].isnull().any():
        ...
        print("  Train unique blocks: " + str(len(train_blocks)))
        print("  Validation unique blocks: " + str(len(val_blocks)))
        print("  Test unique blocks: " + str(len(test_blocks)))
        ...
        if len(train_val_overlap) == 0 and len(train_test_overlap) == 0 and len(val_test_overlap) == 0:
            print("[PASS] No spatial overlap between sets.")
        else:
            print("ERROR: Spatial overlap found!")
            if len(train_val_overlap) > 0:
                print("  Train-Validation overlap: " + str(len(train_val_overlap)) + " blocks")
            if len(train_test_overlap) > 0:
                print("  Train-Test overlap: " + str(len(train_test_overlap)) + " blocks")
            if len(val_test_overlap) > 0:
                print("  Validation-Test overlap: " + str(len(val_test_overlap)) + " blocks")
else:
...
if os.path.exists(preprocessor_path):
    ...
    print("[PASS] Preprocessing pipeline loaded from " + preprocessor_path)
    ...
    if os.path.exists(feature_schema_path):
        ...
        print("[PASS] Feature schema loaded from " + feature_schema_path)
        model_input_features = feature_schema.get('model_input_features', [])
        print("  Model input features (" + str(len(model_input_features)) + "): " + str(model_input_features))
        ...
        for leak in leakage_features:
            if leak in model_input_features:
                print("  ERROR: Leakage feature " + leak + " found in model input features.")
            else:
                print("  [PASS] " + leak + " correctly excluded from model input features.")
        ...
        for field in label_gen_dw_fields:
            if field in model_input_features:
                print("  ERROR: Label-generation DW field " + field + " found in model input features.")
            else:
                print("  [PASS] " + field + " correctly excluded from model input features.")
    else:
...
if all_exist:
    ...
    print("  Model training datetime: " + str(metadata.get('training_datetime', 'N/A')))
    print("  Model type: " + str(metadata.get('model_type', 'N/A')))
    # Check that the feature schema in the metadata matches the one we loaded
    if 'feature_schema_path' in metadata:
        print("  Feature schema path in metadata: " + str(metadata['feature_schema_path']))
    # Check that the preprocessing pipeline path and label encoder path are present
    if 'preprocessing_pipeline_path' in metadata:
        print("  Preprocessing pipeline path in metadata: " + str(metadata['preprocessing_pipeline_path']))
    if 'label_encoder_path' in metadata:
        print("  Label encoder path in metadata: " + str(metadata['label_encoder_path']))
...
if 'metadata' in locals():
    if metadata.get('test_accuracy') is not None and metadata.get('test_macro_f1') is not None:
        print("[PASS] Test set metrics are present in metadata (indicating test was used for final evaluation).")
    else:
    ...
    if 'untouched' in notes.lower() or 'final evaluation' in notes.lower():
        print("[PASS] Notes indicate test set was used for final evaluation only.")
    else:
...
if os.path.exists(model_path) and os.path.exists(preprocessor_path) and os.path.exists(label_encoder_path) and os.path.exists(test_path):
    ...
    if missing_features:
        print("ERROR: Test set missing features: " + str(missing_features))
    else:
        print("[PASS] Test set has all " + str(len(model_input_features)) + " required features.")
        # Prepare features
        ...
        print("  Independently calculated:")
        print("    Accuracy: " + str(round(accuracy, 4)))
        print("    Macro precision: " + str(round(macro_precision, 4)))
        print("    Macro recall: " + str(round(macro_recall, 4)))
        ...
        if 'metadata' in locals():
            ...
            if reported_accuracy is not None and reported_macro_f1 is not None:
                print("  Reported in metadata:")
                print("    Accuracy: " + str(round(reported_accuracy, 4)))
                print("    Macro F1: " + str(round(reported_macro_f1, 4)))
                diff_acc = abs(accuracy - reported_accuracy)
                diff_f1 = abs(macro_f1 - reported_macro_f1)
                print("  Differences:")
                print("    Accuracy: " + str(round(diff_acc, 4)))
                print("    Macro F1: " + str(round(diff_f1, 4)))
                if diff_acc < 1e-4 and diff_f1 < 1e-4:
                    print("[PASS] Independently calculated metrics match reported metrics (within tolerance).")
                else:
...
if os.path.exists(old_verify_path):
    print("[PASS] Old verification script exists: " + old_verify_path)
...
if os.path.exists(old_metadata_path):
    ...
    print("  Old model metadata test accuracy: " + str(old_metadata.get('test_accuracy', 'N/A')))
    print("  Old model metadata test macro F1: " + str(old_metadata.get('test_macro_f1', 'N/A')))
else:
...
if 'feature_schema' in locals():
    print("  Model input features: " + str(feature_schema['model_input_features']))
    print("  Numeric features: " + str(feature_schema['numeric_features']))
    print("  Categorical features: " + str(feature_schema['categorical_features']))
    # Check that the preprocessing steps are as expected
    preprocessing_steps = feature_schema.get('preprocessing_steps', {})
    if preprocessing_steps:
        print("  Preprocessing steps:")
        print("    Numeric: " + str(preprocessing_steps.get('numeric', [])))
        print("    Categorical: " + str(preprocessing_steps.get('categorical', [])))
    else:
    ...
    print("  Feature counts after preprocessing: " + str(feature_schema.get('feature_counts_after_preprocessing', 'N/A')))
else:
...
if os.path.exists(script_path):
    ...
    for cat in five_categories:
        for line in lines:
            if cat in line:
                print("  Found reference to " + cat + " in label generation script.")
                break
else:
...
if os.path.exists(readme_path):
    ...
    if 'SIH five-category' in content or 'five categories' in content:
        print("[PASS] README mentions SIH five-category handling.")
