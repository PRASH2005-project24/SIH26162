"""
Efficient fixed version of weak label generation with temporally correct persistence features.
For each event, persistence features are computed using only observations with
acquisition time <= current event's acquisition time.
Uses grouping and cumulative operations for efficiency.
"""
...
print("Computing temporally correct persistence features using efficient grouping...")
...
for (lat, lon), group in grouped:
    ...
    for i in range(group_size):
        ...
        if len(historical_times) > 0:
            ...
            # active_days: number of unique dates with detections (same as persistence_date_count for this definition)
            active_days[idx] = persistence_date_count[idx]

            # persistence_duration_days: (max_date - min_date).days (same as time_span_days)
            persistence_duration_days[idx] = time_span_days[idx]

            # mean_frp: mean FRP of historical detections
            mean_frp[idx] = np.mean(historical_frp)

            # max_frp: maximum FRP of historical detections
            max_frp[idx] = np.max(historical_frp)
        else:
...
print(f"Processed {processed_rows} rows total")
...
# Now compute labels (same as original)
...
if not leakage_found:
    print("  ✓ No temporal leakage detected in sample test")
else:
    print("  ✗ TEMPORAL LEAKAGE CONFIRMED")