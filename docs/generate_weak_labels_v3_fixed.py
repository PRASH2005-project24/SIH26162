"""
Fixed version of weak label generation with temporally correct persistence features.
...
for i, row in df.iterrows():
    ...
    # Filter to historical observations at the same location (within reasonable spatial tolerance)
    # Using exact match for latitude/longitude as in original code
    historical_mask = (
    ...
    if len(historical_df) > 0:
        ...
        detections_24h[i] = ((historical_df['acq_datetime'] >= time_24h_ago) &
        ...
        detections_3d[i] = ((historical_df['acq_datetime'] >= time_3d_ago) &
                           (historical_df['acq_datetime'] <= current_time)).sum()

        # detections_7d: number of detections in the last 7 days
        time_7d_ago = current_time - timedelta(days=7)
        detections_7d[i] = ((historical_df['acq_datetime'] >= time_7d_ago) &
                           (historical_df['acq_datetime'] <= current_time)).sum()

        # active_days: number of unique dates with detections
        active_days[i] = historical_df['acq_datetime'].dt.date.nunique()

        # persistence_duration_days: (max_date - min_date).days
        persistence_duration_days[i] = time_span_days[i]  # Same as time_span_days

        # mean_frp: mean FRP of historical detections
        mean_frp[i] = historical_df['frp'].mean()

        # max_frp: maximum FRP of historical detections
        max_frp[i] = historical_df['frp'].max()
    else:
